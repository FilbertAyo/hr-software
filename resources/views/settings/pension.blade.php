<x-app-layout>
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-12">

                <div class="row align-items-center mb-3 border-bottom no-gutters">
                    <div class="col">
                        <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home" role="tab"
                                    aria-controls="home" aria-selected="true">pensions</a>
                            </li>

                        </ul>
                    </div>
                    <div class="col-auto">

                        <button type="button" class="btn btn-sm" onclick="reloadPage()">
                            <i class="fe fe-16 fe-refresh-ccw text-muted"></i>
                        </button>
                        <button type="button" class="btn mb-2 btn-primary btn-sm" data-toggle="modal"
                            data-target="#varyModal" data-whatever="@mdo">New pension<span
                                class="fe fe-plus fe-16 ml-2"></span></button>
                    </div>
                </div>

                <div class="row my-2">
                    <!-- Small table -->

                    @include('elements.spinner')
                    <div class="col-md-12">
                        <div class="card shadow-none border">
                            <div class="card-body">

                                <table class="table table-bordered datatables" id="dataTable-1">
                                    <thead class="thead-light">
                                        <tr>
                                            <th>No</th>
                                            <th>Name</th>
                                            <th>Employer %</th>
                                            <th>Employee %</th>
                                            <th>Status</th>
                                            <th class="text-right">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @if ($pensions->count() > 0)
                                            @foreach ($pensions as $index => $pension)
                                                <tr>
                                                    <td>{{ $index + 1 }}</td>
                                                    <td>{{ $pension->name }}</td>
                                                    <td>{{ $pension->employer_percent }}</td>
                                                    <td>{{ $pension->employee_percent }}</td>
                                                    <td>{{ ucfirst($pension->status) }}</td>
                                                    <td class="text-right">
                                                        <div
                                                            style="display: flex; gap: 4px; justify-content: flex-end;">
                                                            <a href="javascript:void(0);"
                                                                class="btn btn-sm btn-primary edit-pension-btn"
                                                                data-pension-id="{{ $pension->id }}"
                                                                data-pension-name="{{ $pension->name }}"
                                                                data-employer-percent="{{ $pension->employer_percent }}"
                                                                data-employee-percent="{{ $pension->employee_percent }}"
                                                                data-status="{{ $pension->status }}">
                                                                <span class="fe fe-edit fe-16"></span>
                                                            </a>

                                                            <form action="{{ route('pension.destroy', $pension->id) }}"
                                                                method="POST"
                                                                onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="btn btn-sm btn-danger">
                                                                    <span class="fe fe-trash-2 fe-16"></span>
                                                                </button>
                                                            </form>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @else
                                            <tr>
                                                <td colspan="6" class="text-center">No pension found</td>
                                            </tr>
                                        @endif
                                    </tbody>
                                </table>


                            </div>
                        </div>
                    </div> <!-- simple table -->


                </div> <!-- .row -->
            </div> <!-- .container-fluid -->


            <div class="modal fade" id="varyModal" tabindex="-1" role="dialog" aria-labelledby="varyModalLabel"
                aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="varyModalLabel">New pension</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form method="POST" action="{{ route('pension.store') }}">
                                @csrf
                                <div class="form-row">
                                    <div class="col-md-12 mb-3">
                                        <label>Name</label>
                                        <input type="text" class="form-control" name="name" required>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label>Employer %</label>
                                        <input type="text" class="form-control" name="employer_percent">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label>Employee %</label>
                                        <input type="text" class="form-control" name="employee_percent">
                                    </div>
                                    <div class="col-md-12 mb-3">
                                        <label>Status</label>
                                        <select class="form-control" name="status">
                                            <option value="active" selected>Active</option>
                                            <option value="inactive">Inactive</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="modal-footer">
                                    <button type="button" class="btn mb-2 btn-secondary"
                                        data-dismiss="modal">Close</button>
                                    <button type="submit" class="btn mb-2 btn-primary">Save and Close</button>
                                </div>
                            </form>
                        </div>


                    </div>
                </div>
            </div>


            <!-- Edit pension Modal -->
            <div class="modal fade" id="editpensionModal" tabindex="-1" role="dialog"
                aria-labelledby="editpensionModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="editpensionModalLabel">Edit pension</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form method="POST" action="" id="editpensionForm">
                                @csrf
                                @method('PUT')
                                <div class="form-row">
                                    <div class="col-md-12 mb-3">
                                        <label>Name</label>
                                        <input type="text" class="form-control" id="editName" name="name"
                                            required>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label>Employer %</label>
                                        <input type="text" class="form-control" id="editEmployerPercent"
                                            name="employer_percent">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label>Employee %</label>
                                        <input type="text" class="form-control" id="editEmployeePercent"
                                            name="employee_percent">
                                    </div>
                                    <div class="col-md-12 mb-3">
                                        <label>Status</label>
                                        <select class="form-control" id="editStatus" name="status">
                                            <option value="active">Active</option>
                                            <option value="inactive">Inactive</option>
                                        </select>
                                    </div>
                                </div>

                                <div class="modal-footer">
                                    <button type="button" class="btn mb-2 btn-secondary"
                                        data-dismiss="modal">Close</button>
                                    <button type="submit" class="btn mb-2 btn-primary">Save Changes</button>
                                </div>
                            </form>
                        </div>

                    </div>
                </div>
            </div>

            <script>
                document.querySelectorAll('.edit-pension-btn').forEach(button => {
                    button.addEventListener('click', function() {
                        const pensionId = this.getAttribute('data-pension-id');
                        const pensionName = this.getAttribute('data-pension-name');
                        const employerPercent = this.getAttribute('data-employer-percent');
                        const employeePercent = this.getAttribute('data-employee-percent');
                        const status = this.getAttribute('data-status');

                        document.getElementById('editpensionForm').setAttribute('action', `/pension/${pensionId}`);
                        document.getElementById('editName').value = pensionName;
                        document.getElementById('editEmployerPercent').value = employerPercent;
                        document.getElementById('editEmployeePercent').value = employeePercent;
                        document.getElementById('editStatus').value = status;

                        $('#editpensionModal').modal('show');
                    });
                });
            </script>



</x-app-layout>
