<x-app-layout>
                            ...
                            <div class="card-body">
                                ...
                                <table class="table table-bordered datatables" id="dataTable-1">
                                    ...
                                    <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                    <div style="display: flex; gap: 4px; justify-content: flex-end;">
                                                        @if($advance->status === 'pending')
                                                            <form action="{{ route('advance.approve', $advance->id) }}" method="POST" style="display: inline;">
                                                                @csrf
                                                                @method('PATCH')
                                                                <button type="submit" class="btn btn-sm btn-success" title="Approve Advance"
                                                                        onclick="return confirm('Are you sure you want to approve this advance?');">
                                                                    <span class="fe fe-check fe-16"></span>
                                                                </button>
                                                            </form>

                                                            <form action="{{ route('advance.reject', $advance->id) }}" method="POST" style="display: inline;">
                                                                @csrf
                                                                @method('PATCH')
                                                                <button type="submit" class="btn btn-sm btn-warning" title="Reject Advance"
                                        ...
                                        @empty
                                            <tr><td colspan="8" class="text-center">No advance found</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                        ...
                        <form method="POST" action="{{ route('advance.store') }}">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <select name="employee_id" id="employeeSelect" class="form-control" required>
                                        ...
                                        @foreach($employees as $emp)
                                                <option value="{{ $emp->id }}" data-limit="{{ $emp->getAdvanceLimit() }}">{{ $emp->employee_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" name="advance_amount" id="advanceAmount" class="form-control" required>
                                    <small id="limitInfo" class="text-muted">Please select an employee to see the advance limit</small>
                                </div>

                                <div class="form-group">
                                    <label>Advance Period</label>
                                    <input type="text" value="{{ $currentPayrollPeriod ? $currentPayrollPeriod->period_name : 'No active period' }}" class="form-control" readonly>
                                    <input type="hidden" name="payroll_period_id" value="{{ $currentPayrollPeriod ? $currentPayrollPeriod->id : '' }}">
                                </div>
                        ...
                        <form method="POST" action="" id="editadvanceForm">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <small id="editLimitInfo" class="text-muted">Please select an employee to see the advance limit</small>
                                </div>
            ...
            <script>
                ...
                document.getElementById('employeeSelect').addEventListener('change', function() {
                    ...
                    if (limit && limit > 0) {
                        limitInfo.textContent = `Maximum advance amount: ${parseFloat(limit).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
