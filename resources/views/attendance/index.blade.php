            ...
            <ul class="nav nav-tabs border-0" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="absent-late-tab" data-toggle="tab" href="#absent-late" role="tab"
                        aria-controls="absent-late" aria-selected="true">Employee Attendance Records</a>
                </li>
            </ul>
        </div>
        ...
        <div class="col-auto">
            @if ($payrollPeriod)
                <div class="btn-group" role="group">
                    ...
                    <a href="{{ route('attendance.bulk-create', ['payroll_period_id' => $payrollPeriod->id]) }}"
                        class="btn btn-success">
                        <i class="fe fe-upload"></i> Bulk Add
                    </a>
                    <a href="{{ route('attendance.export', ['payroll_period_id' => $payrollPeriod->id]) }}"
                        class="btn btn-info">
        ...
        <div class="col-md-12 mb-2">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">Select Payroll Period</h5>
                </div>
                <div class="card-body">
                                ...
                                <select name="payroll_period_id" class="form-control" onchange="this.form.submit()">
                                    ...
                                    @foreach ($payrollPeriods as $period)
                                        <option value="{{ $period->id }}"
                                            ...
                                            {{ $period->period_name }} ({{ $period->start_date->format('M d') }} -
    ...
    @if ($payrollPeriod)
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="align-self-center">
                                <div class="h5 mb-0 text-info">
                                    {{ number_format($attendanceStats['total_deduction_amount'], 2) }}</div>
        ...
        <div class="row">
            <div class="col-md-12 mb-2">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Period: {{ $payrollPeriod->period_name }}  <span
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                ...
                                {{ \Carbon\Carbon::parse($payrollPeriod->start_date)->diffInDaysFiltered(function ($date) {return $date->isWeekday();}, \Carbon\Carbon::parse($payrollPeriod->end_date)) + 1 }}
                            </div>
                            <div class="col-md-3">
                                <strong>Status:</strong>
                                <span
                                    class="badge badge-{{ $payrollPeriod->status === 'completed' ? 'success' : 'warning' }}">
                                            ...
                                            @php
                                                ...
                                                if ($absentDays > 0 || $lateHours > 0) {
                                                    $dailySalary =
                                                        ...
                                                        )->diffInDaysFiltered(function ($date) {
                                                            return $date->isWeekday();
                                                        }, \Carbon\Carbon::parse($payrollPeriod->end_date)) +
                                                            1);
                                                    $deduction =
                                                        $dailySalary * $absentDays + ($dailySalary / 8) * $lateHours;
                                                }
                                            @endphp
                                            @if ($absentDays > 0 || $lateHours > 0)
                                                <tr>
                                                    ...
                                                    <td>{{ $employee->department->department->department_name ?? 'N/A' }}
                                                    </td>
                                                    <td>
                                                        @if ($absentDays > 0)
                                                            <span class="badge badge-danger">{{ $absentDays }}
                                                                days</span>
                                                        @else
                                                    ...
                                                    <td>
                                                        @if ($lateHours > 0)
                                                            <span class="badge badge-warning">{{ $lateHours }}
                                                                hours</span>
                                                        @else
                                                    ...
                                                    <td>
                                                        @if ($deduction > 0)
                                                            <span class="text-danger">TZS
                                                                {{ number_format($deduction, 2) }}</span>
                                                        @else
